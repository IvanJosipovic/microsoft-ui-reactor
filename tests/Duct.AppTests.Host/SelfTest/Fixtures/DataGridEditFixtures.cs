...
/// These mount a real DataGrid with a DuctHost, programmatically trigger editing
    ...
    internal class EditLifecycle(Harness h) : SelfTestFixtureBase(h)
    {
        public override async Task RunAsync()
        {
            ...
            host.Mount(ctx =>
            {
                ...
                {
                    var s = new DataGridState<TestProduct>(source, columns, Duct.DataGrid.SelectionMode.Single);
                    _ = s.LoadDataAsync();
                ...
                return DataGridDsl.DataGrid(
                    source: source,
                    columns: columns,
                    selectionMode: Duct.DataGrid.SelectionMode.Single,
                    editable: true,
    ...
    internal class RapidSelection(Harness h) : SelfTestFixtureBase(h)
    {
        public override async Task RunAsync()
        {
            ...
            host.Mount(ctx =>
            {
                ...
                return VStack(
                    Text($"Selected: {sel.Count}"),
                    DataGridDsl.DataGrid(
                        source: source,
                        columns: CreateEditableColumns(),
                        selectionMode: Duct.DataGrid.SelectionMode.Multiple,
            ...
            host.Mount(ctx =>
            {
                ...
                var columns = new FieldDescriptor[]
                {
                    ...
                    (ColumnDsl.Column<TestProduct>("Name", p => p.Name, width: 160)
                        .CellRenderer(val => Text((string)val).FontSize(fontSize))).Build(),
