...
/// <summary>
/// Phase 2 unit tests: ContextScope, DuctContext, .Provide() modifier,
/// and UseContext hook in isolation. No reconciler, no WinUI controls.
/// </summary>
public class ContextSystemUnitTests
{
    private static readonly DuctContext<string> TestTheme = new("light");
    private static readonly DuctContext<int> TestCount = new(0);
    private static readonly DuctContext<string?> TestSession = new(defaultValue: null);
    ...
    {
        var scope = new ContextScope();
        scope.Push(new Dictionary<DuctContextBase, object?> { [TestTheme] = "dark" });
    ...
    public void ContextScope_Nested_Push_Inner_Shadows_Outer_For_Same_Context()
    {
        var scope = new ContextScope();
        scope.Push(new Dictionary<DuctContextBase, object?> { [TestTheme] = "dark" });
        scope.Push(new Dictionary<DuctContextBase, object?> { [TestTheme] = "high-contrast" });
    ...
    public void ContextScope_Nested_Push_Different_Contexts_Both_Readable()
    {
        var scope = new ContextScope();
        scope.Push(new Dictionary<DuctContextBase, object?> { [TestTheme] = "dark" });
        scope.Push(new Dictionary<DuctContextBase, object?> { [TestCount] = 42 });
    ...
    public void ContextScope_Pop_Restores_Previous_Value()
    {
        var scope = new ContextScope();
        scope.Push(new Dictionary<DuctContextBase, object?> { [TestTheme] = "dark" });
        scope.Push(new Dictionary<DuctContextBase, object?> { [TestTheme] = "blue" });
    ...
    public void ContextScope_Nullable_Value_Works()
    {
        ...
        scope.Push(new Dictionary<DuctContextBase, object?> { [TestSession] = "user123" });
    ...
    public void UseContext_No_Provider_Returns_Default_Value()
    {
        ...
        Assert.Equal("light", value); // DuctContext default
    }
    ...
    // ════════════════════════════════════════════════════════════════
    //  DuctContext type tests
    // ════════════════════════════════════════════════════════════════

    [Fact]
    public void DuctContext_Stores_Default_Value()
    {
        var ctx = new DuctContext<int>(42);
        Assert.Equal(42, ctx.DefaultValue);
    }

    [Fact]
    public void DuctContext_DefaultValueBoxed_Returns_Boxed_Default()
    {
        DuctContextBase ctx = new DuctContext<int>(42);
        Assert.Equal(42, ctx.DefaultValueBoxed);
    }

    [Fact]
    public void DuctContext_DebugName_Uses_CallerMemberName()
    {
        // When defined as a local variable or explicitly named
        var ctx = new DuctContext<string>("default", name: "MyContext");
