...
public class AccessibilityScannerTests
{
    ...
    {
        var tree = VStack(
            Button(Text("🔍"), null) // icon content, no AutomationName
        );
    ...
    {
        var tree = VStack(
            Button(Text("🔍"), null).AutomationName("Search")
        );
    ...
    {
        var tree = VStack(
            Text("Email").AutomationId("EmailLabel"),
    ...
    public void A11Y_004_LargeBoldText_Without_HeadingLevel()
    {
        ...
        var tree = VStack(
            new TextElement("Settings") { FontSize = 24, Weight = new Windows.UI.Text.FontWeight(700) }
        );
    ...
    public void ExportJson_Produces_Valid_File()
    {
        ...
        var tempPath = System.IO.Path.Combine(System.IO.Path.GetTempPath(), $"a11y-test-{Guid.NewGuid()}.json");
        try
        {
    ...
    public void Scanner_Provides_Child_Context_For_IconButton()
    {
        var tree = VStack(
            Button("Reply", null),
            Button("Forward", null),
            Button(Text("🗑"), null) // triggers A11Y_001
        );
