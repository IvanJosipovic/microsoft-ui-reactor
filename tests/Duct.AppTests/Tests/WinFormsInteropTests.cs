...
///
/// These tests launch a real WinForms app with a XAML Island (Duct.WinFormsTests.Host)
/// and drive it through Appium/WinAppDriver to validate:
///   - Tab navigation across the WinForms ↔ WinUI boundary
///   - Rendering of Duct/WinUI controls inside the island
...
///   Right panel (Island):   Duct_TextField1, Duct_Button1, Duct_TextField2
/// </summary>
[TestClass]
public class WinFormsInteropTests : WinFormsTestBase
{
    ...
    {
        // The Duct component's title text should be visible through UIA
        var title = WaitForElement("Duct_Title", timeoutMs: 10000);
        Assert.IsNotNull(title, "Duct title element should be visible in UIA tree");
        Assert.AreEqual("Duct Island Content", title.Text,
    ...
    {
        // Static text that proves the Duct component mounted and rendered
        var proof = WaitForElement("Duct_RenderProof", timeoutMs: 10000);
    ...
    {
        var countText = WaitForElement("Duct_CountDisplay");
    ...
    {
        // Click the Duct button inside the island
        var button = FindById("Duct_Button1");
        button.Click();

        // Counter should increment
        WaitForText("Duct_CountDisplay", "Count: 1");

        // Click again
        button.Click();
        WaitForText("Duct_CountDisplay", "Count: 2");
    }

    [TestMethod]
    public void Interop_Rendering_TextInputWorks()
    {
        // Type into the Duct text field inside the island
        var textField = FindById("Duct_TextField1");
        textField.Click();
        textField.Clear();
        textField.SendKeys("hello island");

        // Verify the display updates
        WaitForText("Duct_TextDisplay", "Text: hello island");
    }
    ...
    /// Duct controls use the .AutomationId() modifier.
    /// </summary>
    ...
    [
        ...
        "Duct_TextField1",   // island — first WinUI control
        "Duct_Button1",      // island — second WinUI control
        "Duct_TextField2",   // island — third WinUI control
    ...
    {
        // Verify Duct controls inside the island are discoverable by AutomationId
        var title = FindById("Duct_Title");
        Assert.IsNotNull(title, "Duct_Title should be findable by AutomationId through UIA");

        var textField = FindById("Duct_TextField1");
        Assert.IsNotNull(textField, "Duct_TextField1 should be findable by AutomationId");

        var button = FindById("Duct_Button1");
        Assert.IsNotNull(button, "Duct_Button1 should be findable by AutomationId");
    }
    ...
    public void Interop_A11y_IslandControlsHaveAccessibleNames()
    {
        ...
        Assert.AreEqual("Island text field", name,
            "Duct TextField should expose its AutomationName through UIA");

        var button = FindById("Duct_Button1");
        var buttonName = button.GetAttribute("Name");
        Assert.AreEqual("Island button", buttonName,
            "Duct Button should expose its AutomationName through UIA");
    }
    ...
    public void Interop_A11y_LiveRegionExposed()
    {
        ...
        var liveRegion = FindById("Duct_LiveRegion");
    ...
    public void Interop_A11y_WinFormsAndIslandControlsBothVisible()
    {
        ...
        var ductButton = FindById("Duct_Button1");
        Assert.IsNotNull(ductButton, "Duct button should be in UIA tree");
        ...
        var ductType = ductButton.GetAttribute("LocalizedControlType");
        Assert.IsNotNull(ductType, "Duct button should have a LocalizedControlType");
    }
