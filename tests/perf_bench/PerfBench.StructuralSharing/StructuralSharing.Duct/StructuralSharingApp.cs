...
namespace PerfBench.StructuralSharing.Duct;
...
public class StructuralSharingApp : Component
{
    ...
    public override Element Render()
    {
        ...
        UseEffect(() =>
        {
            ...
            if (Opts.Headless)
            {
                ...
                shutdown.Tick += (_, _) =>
                {
                    shutdown.Stop();
                    timer.Stop();
                    _tracker.WriteReportFile("EXP3_StructuralSharing_Duct");
        ...
        for (int p = 0; p < PanelCount; p++)
        {
            ...
            items[0] = Text($"Panel {p}").FontSize(12).Bold();
            for (int i = 0; i < ItemsPerPanel; i++)
            {
                items[i + 1] = Text($"Item {i}: {panelData[p][i]:F2}").FontSize(10);
            }
