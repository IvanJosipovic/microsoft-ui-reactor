...
public sealed class MainWindow : Window
{
    ...
    private readonly JournalCellViewModel[] _vms = new JournalCellViewModel[Count];
    ...
    public MainWindow(BenchCliOptions opts)
    {
        ...
        {
            _vms[i] = new JournalCellViewModel();
        ...
        if (_opts.Headless)
        {
            ...
            shutdown.Tick += (_, _) =>
            {
                shutdown.Stop();
                timer.Stop();
                _tracker.WriteReportFile("EXP5_Journal_Bound");
                Close();
            };
            shutdown.Start();
        }
    ...
    private void OnTick(object? sender, object e)
    {
        ...
        for (int i = 0; i < Count; i++)
        {
            ...
            {
                _vms[i].Text = $"{_tick % 1000:D3}";
                mutations++;
                _vms[i].Foreground = (_tick + i) % 2 == 0 ? BrushRed : BrushGreen;
                mutations++;
            }
        }
