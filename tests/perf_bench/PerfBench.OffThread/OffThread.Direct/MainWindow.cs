...
namespace PerfBench.OffThread.Direct;
...
public sealed class MainWindow : Window
{
    ...
    private readonly double[] _values = new double[Count];
    ...
    public MainWindow(BenchCliOptions opts)
    {
        ...
        if (_opts.Headless)
        {
            ...
            shutdown.Tick += (_, _) =>
            {
                shutdown.Stop();
                timer.Stop();
                _tracker.WriteReportFile("EXP4_OffThread_Direct");
                Close();
            };
            shutdown.Start();
        }
    ...
    private void OnTick(object? sender, object e)
    {
        ...
        // Recompute all 1000 values (simulated expensive math)
        for (int i = 0; i < Count; i++)
        {
            double t = _tick * 0.1 + i;
            _values[i] = Math.Sin(t) * Math.Cos(t * 0.7) + Math.Sqrt(Math.Abs(Math.Sin(t * 0.3)));
        }

        // Set all TextBlocks
        for (int i = 0; i < Count; i++)
        {
            _cells[i].Text = $"Item {i}: {_values[i]:F2}";
        }
