...
internal static partial class CompileCommand
{
    public static int Run(string[] args)
    {
        ...
        if (repoRoot == null)
        {
            Console.Error.WriteLine("Error: Could not find repository root (looking for Duct.sln or .git).");
            return 1;
        }
    ...
    private static string? FindRepoRoot()
    {
        ...
        while (dir != null)
        {
            if (File.Exists(Path.Combine(dir, "Duct.sln")) || Directory.Exists(Path.Combine(dir, ".git")))
                return dir;
