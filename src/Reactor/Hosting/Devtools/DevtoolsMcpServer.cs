...
internal sealed class DevtoolsMcpServer : IDisposable
{
    ...
    internal JsonRpcResponse DispatchRpc(string body)
    {
        JsonRpcRequest? request;
        try
        {
            request = JsonSerializer.Deserialize<JsonRpcRequest>(body, JsonOpts);
        }
        ...
        try
        {
            object? result = request.Method switch
            {
                ...
                _ => HandleDirectCall(request.Method, request.Params),
            };
    ...
    private object? HandleDirectCall(string method, JsonElement? @params)
    {
        // Allow callers to invoke tools by their unprefixed name directly, so both
        // `tools/call { name:"version" }` and `{ "method":"version" }` work.
        if (_tools.TryGet(method, out _))
            return InvokeTool(method, @params);

        throw new McpToolException($"Method not found: '{method}'", JsonRpcErrorCodes.MethodNotFound);
    }

    private object? InvokeTool(string name, JsonElement? @params)
    {
