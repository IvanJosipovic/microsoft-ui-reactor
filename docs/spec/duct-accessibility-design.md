# Duct Accessibility System — Detailed Design
...
requires the `.Set()` escape hatch. This blocks WCAG AA compliance for any production Duct
application.

### Current Surface (before this spec)

| WinUI Feature | Duct Status | Duct API |
|---|---|---|
...
1. **Make Duct apps accessible by default** with world-class ergonomics — matching or exceeding
...
2. **Leverage WinUI 3's built-in accessibility** — every standard control already has an
   `AutomationPeer`; Duct just needs to expose the knobs.
3. **WCAG 2.1 AA compliance** as the target for any Duct app that uses the accessibility API
   surface.
...
**Explicit is better than implicit.** Duct will NOT auto-infer accessible names at runtime
...
Duct's accessibility diagnostics emit structured data. Each warning includes the element type,
...
context-aware defaults — without Duct itself doing any guessing.
...
├──────────────────────────────────────────────────────────────────┤
│  Layer 0: WinUI 3 Built-in (already works, no Duct changes)     │
...
Add to the `ElementModifiers` record in `Duct/Core/Element.cs`, alongside the existing
...
Add to `Duct/Elements/ElementExtensions.cs`, in a new `Accessibility Modifiers` section after
...
Extend the modifier application block in `Duct/Core/Reconciler.cs` (after the existing
...
`AutomationProperties.LabeledBy` takes a `UIElement` reference, not a string. Duct's virtual
...
1. The `DuctHost` maintains a hidden, zero-size `TextBlock` in the visual tree with
...
screen reader announcement automatically — no additional Duct logic needed.
...
Duct already has `UseRef<T>()`. Add a `.Ref()` convenience modifier that captures the mounted
...
Duct already has `KeyboardAcceleratorData`. Enhance the reconciler so that when a
...
Duct controls styled with concrete brush values (`.Background("#FF5733")`) will lose their
...
```csharp
DuctApp.Run<MyApp>("App", configure: host =>
{
...
Output via `IDuctLogger` (existing logging interface), tagged `[A11Y]` for filtering:
...
- **`fix.modifier`** tells the agent exactly which Duct modifier to add.
...
|---|---|---|
| `DUCT_A11Y_001` | `Button(iconElement, onClick)` without `.AutomationName()` | Icon-only buttons need an accessible name |
| `DUCT_A11Y_002` | `Image(uri)` without `.AutomationName()` or `.AccessibilityHidden()` | Images need alt text or explicit decoration marking |
| `DUCT_A11Y_003` | `TextField(...)` without `header:` parameter or `.AutomationName()` | Form fields need labels |
...
```csharp
// In test code:
var tree = DuctAccessibilityTree.Capture(host);
...
| Concept | Duct (proposed) | SwiftUI | React / React Aria | Compose |
|---------|----------------|---------|-------------------|---------|
...
- Hidden announcement TextBlock in `DuctHost` (§2.1 implementation)

**Files:** `RenderContext.cs`, `Component.cs`, `DuctHost.cs` / `DuctHostControl.cs`
...
**Files:** new `Duct/Core/AccessibilityDiagnostics.cs`, `DuctHost.cs`

### Phase 7 — Test Helpers (P2)

**Effort:** Low.

- `DuctAccessibilityTree` capture and assertion API (§6.4)

**Files:** new test utility class (possibly in a `Duct.Testing` namespace)

### Phase 8 — Roslyn Analyzers (P2, Stretch)

**Effort:** High.

- Source analyzers for DUCT_A11Y_001–003 (§6.3)

**Files:** new `Duct.Analyzers/` project
...
| File | Changes |
|------|---------|
| `Duct/Core/Element.cs` | ~15 new properties on `ElementModifiers`; update `Merge()`, `ModifiersEqual()` |
| `Duct/Elements/ElementExtensions.cs` | ~20 new fluent modifier extension methods |
| `Duct/Core/Reconciler.cs` | Apply new `AutomationProperties` + keyboard modifiers; `LabeledBy` resolution; skip transitions when reduced motion is on |
| `Duct/Core/RenderContext.cs` | `UseAnnounce()`, `UseHighContrast()`, `UseReducedMotion()`, `UseScreenReaderActive()`, `UseAccessibility()` hooks |
| `Duct/Core/Component.cs` | Expose new hooks as `protected` convenience methods |
| `Duct/Elements/Dsl.cs` | Update `Heading()` / `SubHeading()` defaults; add `FormField()` |
