# Duct Localization System — Design Spec

## 1. Background & Motivation

Duct is a React-inspired declarative UI framework over WinUI 3, using pure C# with no XAML.
...
**XAML access**: `x:Uid="Save"` auto-binds `.Content`, `.Text`, etc. (not usable by Duct)
...
runtime ICU parsing. This is the model we want for Duct.
...
3. **Hook-first API** — `UseLocalization()` hook consistent with Duct's model
...
5. **Rich text support** — localized messages can contain inline formatting (bold, links)
   via tagged ICU syntax mapped to Duct elements
...
XML-like tags that map to Duct element factories:
...
Duct exposes this via modifier overloads:
...
Duct will manage this at the `LocaleProvider` level:
...
Duct should add **logical property modifiers** that resolve based on flow direction,
...
| Concern | How WinUI Handles It | Duct Action Needed |
|---------|--------------------|--------------------|
...
Duct's ICU engine processes them at runtime:
...
[global::System.CodeDom.Compiler.GeneratedCode("Duct.Localization.Generator", "1.0")]
...
`LocaleProvider` is a Duct element that:
...
A command-line tool that scans Duct component source code and manages `.resw` files.
...
code and resource keys. In Duct's model:
...
- In dev mode, Duct watches `.resw` files for changes. When a file changes:
...
To help developers find hardcoded strings that escaped extraction, Duct supports a
**pseudolocalization mode** enabled by a dev flag:

```xml
<!-- In .csproj or launchSettings.json -->
<PropertyGroup>
  <DuctPseudoLocalize>true</DuctPseudoLocalize>
</PropertyGroup>
```
...
2. Log a warning: `[Duct.Intl] Missing key 'Cart.ItemCount' for locale 'fr-FR', falling back to en-US`
...
**Decision**: Duct provides a `UseLocalizedAsset()` hook for developers who want this,
...
- MSBuild integration (`DuctLocMissingKeySeverity`, `DuctPseudoLocalize` properties)
- Pseudolocalization transform: `[!! ŜåvÉ !!]` for localized, `[?? Key ??]` for missing

### Phase 3: CLI — Extract
- Roslyn-based C# AST scanner for Duct DSL patterns (Text, Button, Heading, Placeholder,
