# Duct PropertyGrid — Detailed Design
...
## INPC Support in Duct (Phase 1)

The PropertyGrid needs to observe live mutable objects and re-render when they change. This phase audits the existing observable hooks, identifies gaps, and adds what's needed — as general Duct infrastructure, not PropertyGrid-specific.

### Existing Hooks (Audit)

Duct already has three observable hooks in `RenderContext`:
...
│  │ (metadata)   │    │ (tree model) │    │ (Duct Element) │  │
│  └─────────────┘    └──────────────┘    └────────────────┘  │
│         │                   │                    │            │
...
The provider recognizes both standard `System.ComponentModel` attributes and Duct-specific attributes. The Duct attributes take precedence when both are present.
...
### Phase 1: INPC Foundation (General Duct Infrastructure)
...
- `ReflectionTypeMetadataProvider` with attribute support
  - Duct attributes: `[PropertyCategory]`, `[PropertyDescription]`, `[PropertyDisplayName]`, `[PropertyHidden]`, `[PropertyReadOnly]`, `[PropertyOrder]`, `[PropertyEditor]`
