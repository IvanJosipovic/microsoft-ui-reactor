# Duct Core State & Component Model — Detailed Design
...
The [critical review](../duct-critical-review.md) §2–§3 and scorecard identify three core
...
all score A across the board here. Closing these gaps is prerequisite to Duct being
...
state, localization. Duct has zero mechanism for this. `LocaleContext` proves the pattern
...
### Design: DuctContext\<T\> + .Provide() modifier + UseContext() hook

Duct follows React's mental model, so the API mirrors React Context — but the provide
mechanism uses a modifier (SwiftUI-style) to match Duct's fluent API conventions.
...
public static readonly DuctContext<UserSession?> SessionContext =
    new(defaultValue: null);

// Contexts are typically defined as static fields on a static class:
public static class AppContexts
{
    public static readonly DuctContext<ThemeConfig> Theme = new(ThemeConfig.Light);
    public static readonly DuctContext<UserSession?> Session = new(defaultValue: null);
    public static readonly DuctContext<FeatureFlags> Features = new(FeatureFlags.Default);
}
```
...
/// </summary>
public sealed class DuctContext<T>
{
...
internal sealed class ContextScope
{
    ...
    private readonly List<(DuctContextBase Context, object? Value)> _stack = new();
    // Snapshot version — incremented on every push/pop, used by Memo to detect changes
    private long _version;

    internal void Push(IReadOnlyDictionary<DuctContextBase, object?> values)
    {
    ...
    internal T Read<T>(DuctContext<T> context)
    {
...
### Migration: LocaleContext → DuctContext
...
public static readonly DuctContext<IntlAccessor> LocaleContext =
...
automatic skipping via stable parameters. Duct has nothing — a deeply nested component
...
**Context interaction:**
The reconciler checks whether any `DuctContext` consumed by this component (via
...
internal void FlushEffects()
{
    ...
    for (int i = 0; i < _hooks.Count; i++)
    {
        if (_hooks[i] is EffectHookState hook && hook.PendingCleanup is not null)
        {
            ...
            {
                Debug.WriteLine($"[Duct] Cleanup at index {i} threw: {ex}");
            }
    ...
    for (int i = 0; i < _hooks.Count; i++)
    {
        ...
        {
            Debug.WriteLine($"[Duct] Effect at index {i} threw: {ex}");
        }
    }
}
```
...
Duct doesn't need a special slot mechanism — **Element-typed props are slots.** This
...
```csharp
// Container with single content slot — already idiomatic Duct
VStack(
    Text("Child 1"),
    Text("Child 2")
)
...
1. **DuctContext\<T\> type** — new file `Duct/Core/DuctContext.cs`
2. **ContextValues on Element** — add property to base Element record
3. **.Provide() modifier** — extension method in `Duct/Core/ContextExtensions.cs`
4. **ContextScope in Reconciler** — scope stack, push/pop during traversal
5. **UseContext\<T\> hook** — new hook type in RenderContext
6. **Migrate LocaleContext** — reimplement as DuctContext consumer (non-breaking)
7. **Tests** — context scoping, nesting/shadowing, re-render on change

Estimated scope: New files for DuctContext and extensions. Modifications to Element,
...
|---|---|---|
| `DuctContext<T>` | `Core/DuctContext.cs` | Typed context definition with default value |
...
|---|---|---|
| `UseContext<T>` | `T UseContext<T>(DuctContext<T> context)` | Read nearest provider's value; re-render on change |
...
|---|---|---|
| `.Provide()` | `.Provide<TValue>(DuctContext<TValue> ctx, TValue value)` | Scope a context value to this element's subtree |
