# Duct Data System — Design Specification
...
**PropertyDescriptor** (`Duct/PropertyGrid/PropertyDescriptor.cs`) — describes a single
property with name, display name, type, getter/setter, category, description, order,
and read-only flag.

**TypeMetadata** (`Duct/PropertyGrid/TypeMetadata.cs`) — describes how to edit values
of a given type:
...
**TypeRegistry** (`Duct/PropertyGrid/TypeRegistry.cs`) — maps CLR types to `TypeMetadata`,
...
The validation system (`Duct/Validation/`) provides:
...
2. **DataGridComponent** — Duct component that renders the grid using VirtualList
...
Based on analysis of WinUI.TableView and the existing Duct reconciler, the DataGrid
...
`ItemsStackPanel`. Duct already has `ItemsRepeater` integration via
`DuctElementFactory` (used in the reconciler). ItemsRepeater is the more modern and
flexible choice:

- **Element recycling:** ItemsRepeater creates/recycles elements as they enter/leave
  the viewport. The Duct reconciler's `DuctElementFactory` already bridges this to
...
The standalone `VirtualList` wraps ItemsRepeater integration into a Duct component:
...
- No element recycling pool — ItemsRepeater + DuctElementFactory handle it
...
- Best pattern for Duct: lifecycle-based (Idle → Editing → Validating → Committed)
...
updated to use FieldDescriptor. The `Duct.PropertyGrid` namespace types that reference
`PropertyDescriptor` are migrated. Template delegates (`PropertyRowTemplate`, etc.) are
updated to accept `FieldDescriptor`.

### D2: Virtualizer ships as a general Duct component

**Decision:** Ship `VirtualList` and `VirtualGrid` as first-class standalone components
in `Duct.Virtualization`. DataGrid composes them. They are independently useful for
...
**Implication for Duct DataGrid:** Two viable approaches:

**Option A — Reconciler-driven ListView composition:** The DataGrid element renders into
a WinUI `ListView` (or `ItemsRepeater`), and the Duct reconciler generates
...
**Option B — Pure Duct virtualizer:** Build the virtualizer as a Duct component that
...
ListView) and let WinUI handle element recycling and viewport tracking. The Duct
reconciler already has `ItemsRepeater` integration via `DuctElementFactory`. The
...
into a convenient Duct component. But the hard virtualization work is delegated to
...
- Define `FieldDescriptor` in `Duct.Data` namespace
...
- `VirtualList<T>` Duct component wrapping **ItemsRepeater** (D2, D12)
  - Reconciler integration via existing `DuctElementFactory`
...
| Concept in Duct | Relay | TanStack | AG Grid | MS-Graph | SwiftUI | Compose |
|-----------------|-------|----------|---------|----------|---------|---------|
