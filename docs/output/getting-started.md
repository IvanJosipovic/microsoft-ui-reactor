...
- **`UseState`** returns the current value and a setter. When you call the
...
- **`UseReducer`** is like `UseState` but the setter receives a function
...
- **Components and Props** — Break your app into reusable pieces with
  `Component<TProps>` and typed record props
- **Effects and Lifecycle** — Use `UseEffect` for side effects like timers,
  file I/O, and API calls
- **Collections** — Use `ListView<T>` and `LazyVStack<T>` for virtualized
  scrolling over large datasets
- **Navigation** — Build multi-page apps with `NavigationView` and `TabView`
