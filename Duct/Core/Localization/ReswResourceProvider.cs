...
/// Microsoft.Windows.ApplicationModel.Resources, which relies on the .resw files
...
///     explicit locale choice, which conflicts with Duct's LocaleProvider model
...
///
/// REVISIT: If Duct apps move to packaged deployment (MSIX) as the default,
...
public sealed class ReswResourceProvider : IStringResourceProvider
{
    ...
    private Dictionary<string, string>? GetOrLoadNamespace(string locale, string ns)
    {
        ...
        {
            Debug.WriteLine($"[Duct.Intl] .resw file not found: {reswPath}");
            return null;
        }
    ...
    /// REVISIT: This duplicates parsing logic from Duct.Localization.Generator's
