...
public sealed class IntlAccessor
{
    ...
    public string Asset(string path)
    {
        ...
        var localePath = string.IsNullOrEmpty(dir)
            ? System.IO.Path.Combine(Locale, fileName)
            : System.IO.Path.Combine(dir, Locale, fileName);
    ...
    private string? ResolvePattern(MessageKey key)
    {
        ...
        {
            Debug.WriteLine($"[Duct.Intl] Missing key '{key}' for locale '{Locale}', falling back to {_defaultLocale}");
            pattern = _resourceProvider.GetString(_defaultLocale, key.Namespace, key.Key);
            if (pattern is not null)
                return pattern;
        }

        Debug.WriteLine($"[Duct.Intl] Missing key '{key}' for locale '{Locale}' — no fallback available");
        return null;
    }
