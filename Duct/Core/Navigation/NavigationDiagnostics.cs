...
public static class NavigationDiagnostics
{
    ...
    {
        Debug.WriteLine($"[Duct.Nav] Requested: {mode} from {from} → {to}");
    ...
    {
        Debug.WriteLine($"[Duct.Nav] Completed: {mode} from {from} → {to}");
    ...
    {
        Debug.WriteLine($"[Duct.Nav] Transition START: {transition.GetType().Name} ({mode})");
    ...
    {
        Debug.WriteLine($"[Duct.Nav] Transition END: {transition.GetType().Name} ({mode})");
    ...
    {
        Debug.WriteLine($"[Duct.Nav] DeepLink: '{path}' → {(matched ? $"matched ({routeCount} routes)" : "no match")}");
