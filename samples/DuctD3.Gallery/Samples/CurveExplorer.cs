    ...
    public override Element Render()
    {
        return Func(ctx =>
        {
            ...
            [
                ...
                D3Text(canvasW / 2 - 20, canvasH - 10, "X", 11, Gray(80)),
                D3Text(2, top - 14, "Y", 11, Gray(80)),
            ]);

            return VStack(12,
                HStack(16,
                    HStack(8,
                        Text("Curve:") with { FontSize = 12 },
                        ComboBox(CurveNames, curveIdx, setCurveIdx).Width(160)
                    ).VAlign(VerticalAlignment.Center),
                    HStack(8,
                        (Text("Tension:") with { FontSize = 12 }).Opacity(hasTension ? 1 : 0.4),
                        Slider(tension, 0, 1, setTension)
                            .StepFrequency(0.05)
                            .Width(160)
                            .Disabled(!hasTension),
                        (Text($"{tension:F2}") with { FontSize = 11 }).Opacity(hasTension ? 1 : 0.4)
                    ).VAlign(VerticalAlignment.Center)
                ).Padding(8, 0, 8, 0),
                chart
            ).Padding(12);
