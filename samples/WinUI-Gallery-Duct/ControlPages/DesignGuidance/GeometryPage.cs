...
class GeometryPage : Component
{
    ...
    static Element CornerRadiusResourcesSection(RadiusEntry[] entries) =>
        SampleCard("Corner Radius Resources",
            VStack(12,
                Text("WinUI provides two corner radius theme resources. Using these instead of hard-coded values ensures your UI stays consistent with the system design language and adapts if the values are customized.")
...
Border(dialog).CornerRadius(overlayRadius.TopLeft)");
    ...
    static Element ControlCornerRadiusSection(CornerRadius cr) =>
        SampleCard("ControlCornerRadius",
            VStack(16,
                Text("ControlCornerRadius is used by interactive controls to maintain a consistent, compact rounding. Apply it to buttons, inputs, cards, and other standard controls.")
                    .Foreground(Theme.SecondaryText)
                    .FontSize(13)
                    .Set(tb => tb.TextWrapping = TextWrapping.Wrap)
                    .Margin(0, 0, 0, 4),
                ...
                // Buttons
                Text("Buttons").SemiBold().Foreground(Theme.PrimaryText),
                HStack(12,
                    Button("Standard", () => { })
                ...
                // Text inputs
                Text("Text Input").SemiBold().Foreground(Theme.PrimaryText),
                TextField("", placeholder: "Type here...")
                    .Width(280)
                    .CornerRadius(cr.TopLeft),

                // Card
                Text("Card Surface").SemiBold().Foreground(Theme.PrimaryText),
                Border(
                    VStack(4,
                        Text("Card Title").SemiBold().Foreground(Theme.PrimaryText),
                        Text("Card content using ControlCornerRadius for its border rounding.")
                            .FontSize(13)
                            .Foreground(Theme.SecondaryText)
                            .Set(tb => tb.TextWrapping = TextWrapping.Wrap)
                    ).Margin(16)
                ...
                // Chip / tag
                Text("Tags & Badges").SemiBold().Foreground(Theme.PrimaryText),
                HStack(8,
                    Border(Text("Info").FontSize(12).Foreground(Theme.PrimaryText).Margin(6, 2, 6, 2))
                        .Background(Theme.SubtleFill)
                        .WithBorder(Theme.ControlStroke)
                        .CornerRadius(cr.TopLeft),
                    Border(Text("Success").FontSize(12).Foreground(Theme.PrimaryText).Margin(6, 2, 6, 2))
                        .Background(Theme.SystemSuccessBackground)
                        .WithBorder(Theme.ControlStroke)
                        .CornerRadius(cr.TopLeft),
                    Border(Text("Warning").FontSize(12).Foreground(Theme.PrimaryText).Margin(6, 2, 6, 2))
...
// Tags
Border(Text(""Tag"").Padding(6, 2, 6, 2))
    .Background(Theme.SubtleFill)
    .WithBorder(Theme.ControlStroke)
    .CornerRadius(cr.TopLeft)");
    ...
    static Element OverlayCornerRadiusSection(CornerRadius or) =>
        SampleCard("OverlayCornerRadius",
            VStack(16,
                Text("OverlayCornerRadius is larger than ControlCornerRadius and is used for surfaces that float above the app layer — dialogs, flyouts, menus, teaching tips, and modal overlays.")
                    .Foreground(Theme.SecondaryText)
                    .FontSize(13)
                    .Set(tb => tb.TextWrapping = TextWrapping.Wrap)
                    .Margin(0, 0, 0, 4),
                ...
                // Dialog mock
                Text("Dialog").SemiBold().Foreground(Theme.PrimaryText),
                Border(
                    VStack(12,
                ...
                // Flyout mock
                Text("Flyout / Menu").SemiBold().Foreground(Theme.PrimaryText),
                Border(
                    VStack(2,
                        FlyoutMenuItem("Cut", "\uE8C6"),
                        FlyoutMenuItem("Copy", "\uE8C8"),
                ...
                // Teaching tip mock
                Text("Teaching Tip").SemiBold().Foreground(Theme.PrimaryText),
                Border(
                    VStack(8,
                        Text("Did you know?")
                            .FontSize(14).SemiBold().Foreground(Theme.PrimaryText),
                        Text("You can use OverlayCornerRadius to round any floating surface so it matches the system style for popups and overlays.")
                            .FontSize(13)
                            .Foreground(Theme.SecondaryText)
                            .Set(tb => tb.TextWrapping = TextWrapping.Wrap),
                        Text("Got it").Foreground(Theme.AccentText).FontSize(13)
                    ).Margin(20)
                )
                .Background(Theme.LayerFill)
                .WithBorder(Theme.SurfaceStroke)
...
Border(tipContent)
    ...
    static Element MixedRadiiSection(CornerRadius cr, CornerRadius or) =>
        SampleCard("Comparing Radii",
            VStack(12,
                Text("Nesting controls inside overlay surfaces is a common pattern. The outer container uses OverlayCornerRadius, while the inner controls use ControlCornerRadius.")
                    .Foreground(Theme.SecondaryText)
                    .FontSize(13)
                    .Set(tb => tb.TextWrapping = TextWrapping.Wrap)
                    .Margin(0, 0, 0, 4),
