...
class SpacingPage : Component
{
    public override Element Render()
    {
        return PageContent("Spacing",
            "WinUI uses a 4px base grid and consistent spacing values to create hierarchy, group related content, and maintain visual rhythm. Duct exposes Margin and Padding modifiers that map directly to WinUI layout properties.",
    ...
    static Element SpacingScaleSection()
    {
        ...
        return SampleCard("Spacing Scale",
            VStack(8,
                Text("WinUI's spacing scale is built on a 4px base unit. Consistent use of these values creates a clear visual hierarchy.")
    ...
    static Element MarginVsPaddingSection() =>
        SampleCard("Margin vs Padding",
            VStack(16,
                Text("Margin adds space outside an element; Padding adds space inside. In Duct, .Padding() only works on Border and Control elements (Button, TextField, etc.). Layout panels like VStack and HStack only support .Margin() — wrap content in a Border if you need inner padding on a stack.")
                    .Foreground(Theme.SecondaryText)
                    .FontSize(13)
                    .Set(tb => tb.TextWrapping = TextWrapping.Wrap)
                    .Margin(0, 0, 0, 4),
                ...
                Grid(
                    ...
                    // Margin demo
                    VStack(4,
                        Text("Margin").SemiBold().Foreground(Theme.PrimaryText),
                        Text("Space outside the border").FontSize(12).Foreground(Theme.SecondaryText),
                        Border(
                            Border(
                                Text("Content").Foreground(Theme.PrimaryText)
                        ...
                        .Set(b =>
                        {
                            ...
                            border.BorderBrush = new Microsoft.UI.Xaml.Media.SolidColorBrush(
                                Windows.UI.Color.FromArgb(128, 255, 150, 50));
                        })
                        .CornerRadius(6)
                        .Height(80)
                    ...
                    // Padding demo
                    VStack(4,
                        Text("Padding").SemiBold().Foreground(Theme.PrimaryText),
                        Text("Space inside the border").FontSize(12).Foreground(Theme.SecondaryText),
                        Border(
                            Text("Content").Foreground(Theme.PrimaryText)
                                .HAlign(HorizontalAlignment.Center)
                                .VAlign(VerticalAlignment.Center)
                        ...
                        .Set(b =>
                        {
                            var border = (Microsoft.UI.Xaml.Controls.Border)b;
                            border.BorderBrush = new Microsoft.UI.Xaml.Media.SolidColorBrush(
                                Windows.UI.Color.FromArgb(128, 100, 180, 255));
                        })
                ...
                // Compatibility table
                Text("Compatibility").SemiBold().Foreground(Theme.PrimaryText),
                VStack(2,
...
Border(
    ...
    static Element CompatRow(string element, bool margin, bool padding) =>
        Grid(
            ...
            Text(element).FontSize(13).Foreground(Theme.PrimaryText)
    ...
    static Element MarginSection() =>
        SampleCard("Margin",
            VStack(12,
                Text("Margin pushes an element away from its neighbors. It works on every Duct element.")
                    .Foreground(Theme.SecondaryText)
                    .FontSize(13)
                    .Set(tb => tb.TextWrapping = TextWrapping.Wrap)
                    .Margin(0, 0, 0, 4),
...
element.Margin(4, 8, 16, 24)");
    ...
    static Element MarginDemo(string label, double l, double t, double r, double b) =>
        VStack(2,
            Text(label).FontSize(12).Foreground(Theme.SecondaryText),
            Border(
                Border(
                    Text($"{l}, {t}, {r}, {b}")
                        .FontSize(11)
    ...
    static Element PaddingSection() =>
        SampleCard("Padding",
            VStack(12,
                Text("Padding adds inner space between a container's edge and its content. In Duct it applies to Border and Control-based elements only.")
                    .Foreground(Theme.SecondaryText)
                    .FontSize(13)
                    .Set(tb => tb.TextWrapping = TextWrapping.Wrap)
                    .Margin(0, 0, 0, 4),
...
Border(child).Padding(4, 8, 16, 24)");
    ...
    static Element StackSpacingSection() =>
        SampleCard("Stack Spacing",
            VStack(16,
                Text("VStack and HStack accept a spacing parameter that inserts uniform gaps between children. This is the recommended way to space siblings — use it instead of adding margins to individual items.")
                    .Foreground(Theme.SecondaryText)
                    .FontSize(13)
                    .Set(tb => tb.TextWrapping = TextWrapping.Wrap)
                    .Margin(0, 0, 0, 4),

                Text("VStack spacing").SemiBold().FontSize(13).Foreground(Theme.PrimaryText),
                HStack(24,
                    StackDemo("0", 0, true),
                    StackDemo("4", 4, true),
                    StackDemo("8", 8, true),
                ...
                Text("HStack spacing").SemiBold().FontSize(13).Foreground(Theme.PrimaryText),
                VStack(8,
                    StackDemo("0", 0, false),
                    StackDemo("4", 4, false),
                    StackDemo("8", 8, false),
...
HStack(12,
    ...
    static Element StackDemo(string label, int spacing, bool vertical)
    {
        ...
        return VStack(2,
            Text($"{label}px").FontSize(11).Foreground(Theme.SecondaryText),
            Border(stack)
                .Background(Theme.SubtleFill)
                .WithBorder(Theme.DividerStroke)
                .CornerRadius(6)
    ...
    static Element PageLayoutSection() =>
        SampleCard("Page Layout Spacing",
            VStack(12,
                Text("A typical page combines all spacing techniques: page-level margins, section spacing in stacks, card padding via Border, and control gaps. Here is a common pattern:")
                    .Foreground(Theme.SecondaryText)
                    .FontSize(13)
                    .Set(tb => tb.TextWrapping = TextWrapping.Wrap)
                    .Margin(0, 0, 0, 4),
                ...
                Border(
                    VStack(16,
                        // Header
                        VStack(4,
                            Text("Page Title").FontSize(18).SemiBold().Foreground(Theme.PrimaryText),
                            Text("A brief description of the page content.")
                        ...
                        Border(
                            VStack(8,
                                Text("Section Card").SemiBold().Foreground(Theme.PrimaryText),
                                Text("Content inside a card uses 16px padding via the Border wrapper.")
                                    .FontSize(13).Foreground(Theme.SecondaryText)
                                    .Set(tb => tb.TextWrapping = TextWrapping.Wrap),
                                HStack(8,
